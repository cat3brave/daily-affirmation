...
- ユーザーが変更とcommitを明示的に依頼した場合は、依頼された1つの目的を1つのPR単位として扱う。
- 明示的に許可された範囲内で、調査、実装、必要なテスト追加、安全な修正、このAGENTS.mdで定める検証、commitまで連続して進める。
- 正常な進行についてユーザーの中間承認を求めず、応答待ちで作業を停止しない。
- 無関係なリファクタリング、整理、機能追加、依存関係更新を含めない。
- 必須検証が許可範囲内で安全に修正できる場合は、修正後に必要な検証だけを再実行する。
- 次の場合だけ停止し、正確な理由を報告する。
  - 要件に、結果を大きく変える複数の解釈がある
  - 完了に、ユーザーが許可していないファイルまたは操作が必要
  - 必須検証の失敗を、許可範囲内で安全に解消できない
  - 実環境の認証・権限設定、RLSまたはデータベース構造、本番データ、秘密情報の追加・開示・変更、破壊的操作が必要
  - merge conflict、作業開始前から存在する想定外の差分、または無関係な差分が見つかった
- commit後は、このファイルの「Codex Cloud」ルールに従ってpush、PR作成、auto-merge、Verify監視を続行する。
- 完了時は、変更内容、検証結果、変更ファイル、commit SHA、PR・CI・merge結果（該当する場合）、停止理由または「なし」だけを報告する。
